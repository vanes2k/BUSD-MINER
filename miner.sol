...
contract BusdCrops {
    ...
    uint256 public GEMS_TO_HIRE_1DIGGERS = 1080000;
    uint256 public PERCENTS_DIVIDER = 1000;
    uint256 public REFERRAL = 150;
    uint256 public TAX = 12;
    uint256 public MARKET_GEMS_DIVISOR = 2; // 50%
    uint256 public MARKET_GEMS_DIVISOR_SELL = 1; // 100%
    ...
    function hatchGEMS(bool isCompound) public {
        User storage user = users[msg.sender];
        require(contractStarted, "Contract not yet Started.");

        uint256 GEMSUsed = getMyGEMS();
        uint256 GEMSForCompound = GEMSUsed;

        if(isCompound) {
            uint256 dailyCompoundBonus = getDailyCompoundBonus(msg.sender, GEMSForCompound);
            GEMSForCompound = GEMSForCompound.add(dailyCompoundBonus);
            uint256 GEMSUsedValue = calculateGEMSell(GEMSForCompound);
            user.userDeposit = user.userDeposit.add(GEMSUsedValue);
            totalCompound = totalCompound.add(GEMSUsedValue);
        } 
        ...
        user.DIGGERS = user.DIGGERS.add(GEMSForCompound.div(GEMS_TO_HIRE_1DIGGERS));
        user.claimedGEMS = 0;
        user.lastHatch = block.timestamp;

        marketGEMS = marketGEMS.add(GEMSUsed.div(MARKET_GEMS_DIVISOR));
    }

    function sellGEMS() public{
        require(contractStarted);
        User storage user = users[msg.sender];
        uint256 hasGEMS = getMyGEMS();
        uint256 eggValue = calculateGEMSell(hasGEMS);
        ...
        user.claimedGEMS = 0;  
        user.lastHatch = block.timestamp;
        marketGEMS = marketGEMS.add(hasGEMS.div(MARKET_GEMS_DIVISOR_SELL));
        
        if(getBalance() < eggValue) {
            eggValue = getBalance();
        }

        uint256 GEMSPayout = eggValue.sub(payFees(eggValue));
        token_BUSD.transfer(msg.sender, GEMSPayout);
        user.totalWithdrawn = user.totalWithdrawn.add(GEMSPayout);
        totalWithdrawn = totalWithdrawn.add(GEMSPayout);
    }

    function buyGEMS(address ref, uint256 amount) public{
        require(contractStarted);
        User storage user = users[msg.sender];
        ...
        uint256 GEMSBought = calculateEggBuy(amount, getBalance().sub(amount));
        user.userDeposit = user.userDeposit.add(amount);
        user.initialDeposit = user.initialDeposit.add(amount);
        user.claimedGEMS = user.claimedGEMS.add(GEMSBought);
        ...
        uint256 GEMSPayout = payFees(amount);
        /** less the fee on total Staked to give more transparency of data. **/
        totalStaked = totalStaked.add(amount.sub(GEMSPayout));
    ...
    function getUserInfo(address _adr) public view returns(uint256 _initialDeposit, uint256 _userDeposit, uint256 _DIGGERS,
     uint256 _claimedGEMS, uint256 _lastHatch, address _referrer, uint256 _referrals,
	 uint256 _totalWithdrawn, uint256 _referralEggRewards, uint256 _dailyCompoundBonus, uint256 _lastWithdrawTime) {
         _initialDeposit = users[_adr].initialDeposit;
         _userDeposit = users[_adr].userDeposit;
         _DIGGERS = users[_adr].DIGGERS;
         _claimedGEMS = users[_adr].claimedGEMS;
    ...
    function initialize(uint256 amount) public{
        if (!contractStarted) {
    		if (msg.sender == owner) {
                ...
                marketGEMS = 86400000000;
                buyGEMS(msg.sender, amount);
    ...
    function getAvailableEarnings(address _adr) public view returns(uint256) {
        uint256 userGEMS = users[_adr].claimedGEMS.add(getGEMSSinceLastHatch(_adr));
        return calculateGEMSell(userGEMS);
    }
    ...
    function calculateGEMSell(uint256 GEMS) public view returns(uint256){
        return calculateTrade(GEMS, marketGEMS, getBalance());
    }
    ...
    function getGEMSYield(uint256 amount) public view returns(uint256,uint256) {
        uint256 GEMSAmount = calculateEggBuy(amount , getBalance().add(amount).sub(amount));
        uint256 DIGGERS = GEMSAmount.div(GEMS_TO_HIRE_1DIGGERS);
        uint256 day = 1 days;
        uint256 GEMSPerDay = day.mul(DIGGERS);
        uint256 earningsPerDay = calculateGEMSellForYield(GEMSPerDay, amount);
        return(DIGGERS, earningsPerDay);
    }

    function calculateGEMSellForYield(uint256 GEMS,uint256 amount) public view returns(uint256){
        return calculateTrade(GEMS,marketGEMS, getBalance().add(amount));
    }
    ...
    function getMyDIGGERS() public view returns(uint256){
        return users[msg.sender].DIGGERS;
    }

    function getMyGEMS() public view returns(uint256){
        return users[msg.sender].claimedGEMS.add(getGEMSSinceLastHatch(msg.sender));
    }

    function getGEMSSinceLastHatch(address adr) public view returns(uint256){
        uint256 secondsSinceLastHatch = block.timestamp.sub(users[adr].lastHatch);
                            /** get min time. **/
        uint256 cutoffTime = min(secondsSinceLastHatch, CUTOFF_STEP);
        uint256 secondsPassed = min(GEMS_TO_HIRE_1DIGGERS, cutoffTime);
        return secondsPassed.mul(users[adr].DIGGERS);
    }
    ...
    function PRC_GEMS_TO_HIRE_1DIGGERS(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        require(value >= 479520 && value <= 2592000); /** min 3% max 12%**/
        GEMS_TO_HIRE_1DIGGERS = value;
    }
    ...
    function PRC_MARKET_GEMS_DIVISOR(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        require(value <= 50); /** 50 = 2% **/
        MARKET_GEMS_DIVISOR = value;
    }
